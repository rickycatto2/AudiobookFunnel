...
function drawQueue() {
  const states = ["REVIEW", "READY", "PROCESSING", "COMPLETE", "ERROR"];
  $("#counts").innerHTML = states
    .map(
      (s) =>
        `<div class="count"><span>${{ REVIEW: "Needs review", READY: "Ready / scheduled", PROCESSING: "Processing", COMPLETE: "In your library", ERROR: "Needs attention" }[s]}</span><strong>${snapshot.jobs.filter((j) => j.status === s).length}</strong></div>`,
    )
    .join("");
